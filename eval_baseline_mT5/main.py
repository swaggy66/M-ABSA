...
def init_args():
    ...
    parser.add_argument("--evaluate_checkpoint_dir",default="",type=str)

    # language
    parser.add_argument("--lang",default="en",type=str)
    parser.add_argument("--sourcelang",default="zh",type=str)
    parser.add_argument("--sourcedataset",default="res_acl_uabsa",type=str)
...
if args.do_eval:
    ...
    saved_model_dir = args.evaluate_checkpoint_dir
    ...
    for checkpoint in all_checkpoints:
        ...
        if 0 <= int(epoch) < 100:
            ...
            # #print(dev_result['f1'],type(dev_result['f1']))
            # #dev_result['f1'] = int(dev_result['f1'])
            # if dev_result[1]['f1'] > best_f1:
            #     best_f1 = dev_result[1]['f1']
            #     best_checkpoint = checkpoint
            #     best_epoch = epoch
            dev_result_raw,dev_result_fix = evaluate(dev_loader, model, args.paradigm, args.task,sents)
            #print(dev_result['f1'],type(dev_result['f1']))
            #dev_result['f1'] = int(dev_result['f1'])
            if dev_result_fix['f1'] > best_f1:
                best_f1 = dev_result_fix['f1']
                best_checkpoint = checkpoint
                best_epoch = epoch

            # add the global step to the name of these metrics for recording
            # 'f1' --> 'f1_1000'
            print('111',dev_result_fix)
            # dev_result = dict((k + '_{}'.format(epoch), v) for k, v in dev_result.items())
            # dev_results.update(dev_result)
            # dev_result_raw = dict((k + '_{}'.format(epoch), v) for k, v in dev_result_raw.items())
            # dev_results.update(dev_result_raw)
            dev_result_fix = dict((k + '_{}'.format(epoch), v) for k, v in dev_result_fix.items())
            dev_results.update(dev_result_fix)
            sents1, _ = read_line_examples_from_file(f'data/{args.task}/{args.dataset}/test.txt')

            test_result_raw,test_result_fix = evaluate(test_loader, model, args.paradigm, args.task,sents1)
            test_result_fix = dict((k + '_{}'.format(epoch), v) for k, v in test_result_fix.items())
            test_results.update(test_result_fix)
    ...
    # log_file_path = f"{results_log_dir}/{args.task}-cross_domain_{args.sourcelang}_{args.lang}.txt"
